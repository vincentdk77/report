...
import com.jiatuobao.mongo.MongoOpLogUtilIncrementShard1.nsLists
...
//            try {
      ...
      val query: BasicDBObject = new BasicDBObject("ts", new BasicDBObject("$gt", queryTs))
  ...
  private def resembleJsonFields(document1: Document, database: String, tableName: String, op: String) = {
    ...
    val _id: String = document1.getObjectId("_id").toHexString
