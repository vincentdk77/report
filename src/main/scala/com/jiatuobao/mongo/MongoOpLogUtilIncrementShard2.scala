...
object MongoOpLogUtilIncrementShard2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mongoClient1: MongoClient = MyMongoUtil.getMongoShard1Client
    val mongoClient2: MongoClient = MyMongoUtil.getMongoShard2Client

    OpLogTest(jedis, mongoClient2, mongoClient1)
  }


  def OpLogTest(jedis: Jedis, mongoClient2: MongoClient,mongoClient1:MongoClient): Unit = {

    //获取命名空间list
    val collectionNames: MongoIterable[String] = mongoClient1.getDatabase("crm").listCollectionNames
    ...
    val opLogCollection: MongoCollection[Document] = mongoClient2.getDatabase("local").getCollection("oplog.rs")
      ...
      //            try {
        ...
        if (opDocuments.hasNext) {
          ...
          while ( opDocuments.hasNext) {
            var message = ""
            val document: Document = opDocuments.next
            ...
            if (op == "u") {
              ...
              val collection: MongoCollection[Document] = mongoClient2.getDatabase(database).getCollection(realTableName)
            ...
            if (op == "d") {
              where = context
              val testCollection: MongoCollection[Document] = mongoClient2.getDatabase(database).getCollection(realTableName)
